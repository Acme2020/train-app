...
describe("filterByDuration", () => {
  const MOCK_NOW = new Date("2025-09-16T12:00:00.000Z").getTime();
  beforeAll(() => {
  ...
  const entries = [
    { when: new Date(MOCK_NOW + 1 * 60 * 1000).toISOString(), id: "a" }, // +1 min
    { when: new Date(MOCK_NOW + 5 * 60 * 1000).toISOString(), id: "b" }, // +5 min
    { when: new Date(MOCK_NOW + 15 * 60 * 1000).toISOString(), id: "c" }, // +15 min
    { when: new Date(MOCK_NOW - 5 * 60 * 1000).toISOString(), id: "d" }, // -5 min
